...
export default function Home() {
  ...
  const [role, setRole] = useState<'trader' | 'investor'>('trader');
  ...
  const handleCreateUser = (e: React.FormEvent) => {
    ...
    if (name && email && walletAddress) {
      createUserMutation.mutate({ name, email, walletAddress, role });
    }
  };
        ...
        <div className="bg-white rounded-lg shadow-md p-6">
          ...
          <form onSubmit={handleCreateUser} className="mb-8 p-4 bg-gray-50 rounded">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
              ...
              <select
                value={role}
                onChange={(e) => setRole(e.target.value as 'trader' | 'investor')}
                className="px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                required
              >
                  ...
                  usersQuery.data?.map((user) => (
                    <div key={user._id} className="flex items-center justify-between p-3 bg-gray-50 rounded">
                      <div>
                        ...
                        <p className="text-gray-500">Wallet: {user.walletAddress}</p>
